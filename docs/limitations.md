...
- SQLite and PostgreSQL are supported; production rate limiting is distributed through Redis. Redis-backed caching is implemented for incident lookups and can be extended to other read-heavy paths.
...
- A timed-out remote request may continue provider transport cancellation cleanup, but its result is ignored and deterministic fallback preserves the original run ID.
